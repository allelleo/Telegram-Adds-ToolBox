...
async def get_users(limit, offset):
    async with httpx.AsyncClient() as client:
        response = await client.get(f"{BACKEND_URL}/api/v1/user/get-user-with-pagination-with-username?limit={limit}&offset={offset}")
        print(response.json())
    return response.json()

async def send_status(data):
    async with httpx.AsyncClient() as client:
        response = await client.post(f"{BACKEND_URL}/api/v1/user/set-user-online-status", json={"data": data})
        print(response.json())
        ...
        for entity in data:
            try:
                ...
                if isinstance(status, UserStatusOnline):
                    ...
                    satus_data.append(
                        {'telegram_id': entity['telegram_id'], 'status': datetime(
                            int(year), int(month), int(day), int(hour), int(minute), int(second)
                        )}
                    )
                    print(f"{user.first_name or 'User'} [{user.id}]: 🔴 Был(а) в сети: {status.was_online}")
                    ...
                    print(f"{user.first_name or 'User'} [{user.id}]: 🕒 Был(а) в течение недели")
                elif isinstance(status, UserStatusLastMonth):
                    satus_data.append(
                        {'telegram_id': entity['telegram_id'], 'status': 'month'}
                    )
                    print(f"{user.first_name or 'User'} [{user.id}]: 📆 Был(а) в течение месяца")
                else:
                    satus_data.append(
                        {'telegram_id': entity['telegram_id'], 'status': 'unknown'}
                    )
                    print(f"{user.first_name or 'User'} [{user.id}]: ❔ Статус неизвестен")
